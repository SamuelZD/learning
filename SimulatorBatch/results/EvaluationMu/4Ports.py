...
# LOAD DATA FILES
filename = ["1limitedmodel3","1limitedsim3"]
...
for i in range(N):
	plot(lamb, q[0][i], colors[i] + types[0] , markersize = 8 , linewidth = 1, label = "approx "+ ": $\overline{q}_" + str(i+1) + "$")
	plot(lamb, q[1][i], colors[i] + types[1] , markersize = 8 , linewidth = 1, label = "simu "+ ": $\overline{q}_" + str(i+1) + "$")

xlabel("Load (Mpps)", fontsize=25)
ylabel("Average Queue Size", fontsize=20)
...
for i in range(N):
	plot(lamb, b[0][i], colors[i] + types[0] , markersize = 8 , linewidth = 1, label =  "approx "+ ": $\overline{b}_" + str(i+1) + "$")
	plot(lamb, b[1][i], colors[i] + types[1], markersize = 8 , linewidth = 1, label =  "simu "+ ": $\overline{b}_" + str(i+1) + "$")

xlabel("Load (Mpps)", fontsize=25)
ylabel("Loss Rate", fontsize=20)
...
for i in range(N):
	plot(lamb, 1/mu[0][i], colors[i] + types[0] , markersize = 8 , linewidth = 1, label = "approx " + " : $\mu_" + str(i+1) + "$")
	plot(lamb, 1/mu[1][i], colors[i] + types[1] , markersize = 8 , linewidth = 1, label = "simu "+ " : $\mu_" + str(i+1) + "$")
xlabel("Load (Mpps)", fontsize=25)
