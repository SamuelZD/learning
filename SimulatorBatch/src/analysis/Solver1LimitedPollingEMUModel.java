 ...
 * 
 * Proposed by Bruno
 * no function now testing now
 * 
 */
public class Solver1LimitedPollingEMUModel {
	...
	private Vector<double[]> probaStrange;
	
	private int type = 1; // the type of formula, by defaut one Miss
	
	// Solver constructor (before any solver initialization)	
	public Solver1LimitedPollingEMUModel() {
		this.N = 0;
		this.queues = new Vector<QueueInfo>();
		this.probaIns = new Vector<double[]>();
		this.probaStrange = new Vector<double[]>();
	}
	
	public Solver1LimitedPollingEMUModel(int m, double tm, double th, double tr) {
		this.N = 0;
		this.queues = new Vector<QueueInfo>();
		this.M = m;
		this.timeH = th;
	 ...
	 * add the probas instant
	 */
	public void addProbasInstantStrange(double[] probas)
	{
		this.probaStrange.add(probas);
	}
	...
	/**
	 * Comput the average batch size
	 * k = 1 or 2
	 */
	private double[] computeAverageMandTime( int k, QueueInfo q)
	{
		double[] m_t = new double[2];
		double m = 0 ;
		double time = 0;
		
		//get the sum for renormalise
		double sum = 0;
		for(int i = k ; i <= q.getK(); i++ )
		{
			sum += (q.getState(i, 0).getProba() + q.getState(i, 1).getProba());
		}
		
		//get the average batch size
		for(int i = k; i <= q.getK() ; i++)
		{
			if (i < this.M) m += i * (q.getState(i, 0).getProba()/ sum + q.getState(i, 1).getProba()/ sum)  ;
			else m += M * (q.getState(i, 0).getProba()/ sum + q.getState(i, 1).getProba()/ sum)  ;
		}
		...
		for(int i = k; i<= q.getK() ; i++)
		{
			debug += (q.getState(i, 0).getProba()/sum + q.getState(i, 1).getProba()/sum);
		}
		System.out.println("normalization : " + debug);
	
		/*
		 * my idea */
		if(m < this.M) m += this.M /3;
		if(m > this.M) m = this.M;
		
		time = (this.timeM * k + this.timeH * (m - k) )/m + this.timeR;
	...
	/**
	 * Computing the average batch size
	 * @param q_ port NO.
	 * @return the average batch size
	 */
	private double computeAverageBatchSize(int q_)
	{
		double m = 0;
		...
		double ps = 0;
		for(int k = 1; k <= q.getK(); k++)
		{
			ps += q.getState(k, 1).getProba();
		}
		
		for (int k = 0 ; k <= q.getK() ; k++)
		{
			double tempM = (k >= this.M) ? this.M:k;
			//m += tempM*q.getState(k, 1).getProba()/ps;
			m += tempM*this.probaIns.get(q_)[k];
		}
	...
	/**
	 * accurate inferior formula
	 * conditionne
	 * @param q_
	 * @return 
 	 */
	private double accurateInferior(int q_)
	{
		...
		for (int i = 1 ; i <= q.getK(); i++)
		{
			...
			if (i >= M)
				...
				k = i;
			double prob = (q.getState(i, 0).getProba() + q.getState(i, 1).getProba())/(1 - q.getState(0, 0).getProba() - q.getState(0, 1).getProba());//condition

			
			firstPart = prob*(this.timeM + (k - 1)*this.timeH)/k;
			serviceTime += firstPart;
		}
		

		//return serviceTime + this.timeR*(1 - this.probaIns.get(q_)[0]);///(1 - this.probaIns.get(q_)[0]);
		return serviceTime + this.timeR;
		
		//return serviceTime + this.timeR*(1 - this.probaIns.get(q_)[0]);
	}
	
	/**
	 * accurate superior formulao
	 * @param q_
	 * @return
	 */
	private double accurateSuperior(int q_)
	{
		...
		{
			ps += q.getState(k, 1).getProba();
			ps += q.getState(k, 0).getProba();
		}
		
		for (int i = 2 ; i <= q.getK(); i++)
		{
			double firstPart = 0;
			double k = i;
			if (i >= M) k = M;
			else k = i;
			//double prob = this.probaIns.get(q_)[i]/(1 - this.probaIns.get(q_)[0] - this.probaIns.get(q_)[1]);//condition
			//double prob = (q.getState(i, 0).getProba() + q.getState(i, 1).getProba())/(1 - q.getState(0, 0).getProba() - q.getState(0, 1).getProba());//condition
			double prob = (q.getState(i, 0).getProba() + q.getState(i, 1).getProba())/ps;//condition
		
			firstPart = prob*(2*this.timeM + (k - 2)*this.timeH)/k;
			serviceTime += firstPart;
		}
		// condition also for P_p,A (1)
		//return serviceTime + this.timeR + (q.getState(1, 0).getProba() + q.getState(1, 1).getProba() )/(1 - q.getState(0, 0).getProba() - q.getState(0, 1).getProba())*this.timeM;
		return serviceTime + this.timeR + (q.getState(1, 0).getProba() + q.getState(1, 1).getProba() )/ps*this.timeM;
	}
	...
	private double approximationInferior(int q_)
	{
		...
		for (int i = 1; i <= q.getK(); i++) {
			...
			if (i >= M)
				...
				k = i;
			double prob = (q.getState(i, 0).getProba() + q.getState(i, 1).getProba())/(1 - q.getState(0, 0).getProba() - q.getState(0, 1).getProba());
			m += prob * k;
		}
...
//		serviceTime += (2*this.timeM + second*this.timeH)/m + this.timeR;
		...
		//double serviceTimeSup = this.accurateSuperior(q_);
		serviceTime = 2*proba*serviceTimeInf + (1 - 2*proba)*serviceTimeSup;
		
		return serviceTime;
	}
	
	/**
	 * piecewise inferior formula
	 * non conditionné
	 * @param q_
	 * @return
	 */
	private double piecewiseInferior(int q_)
	{
		double serviceTime = 0;
		QueueInfo q = this.queues.get(q_);
		...
		for (int i = 1; i <= q.getK(); i++) {
			...
			double prob = (this.probaIns.get(q_)[i]);
			m += prob * k;
		}
		double min = (m>1)?1:m, max = (m-1 > 0)?(m-1):0;
		serviceTime += (min*this.timeM + max*this.timeH)/m + this.timeR;
	...
	/**
	 * piecewise superior formula
	 * @param q_
	 * @return
	 */
	private double piecewiseSuperior(int q_)
	{
		double serviceTime = 0;
		QueueInfo q = this.queues.get(q_);
		...
		double min = (m>2)?2:m, max = (m-2 > 0)?(m-2):0;
		serviceTime += (min*this.timeM + max*this.timeH)/m + this.timeR;
	...
	public double computeProcessingRate(int q_)
	{
		...
		{
			case 11:{
				serviceTime = this.accurateInferior(q_);
				break;
			}
			case 12:{
			...
			case 13:{
				serviceTime = this.piecewiseInferior(q_);
				break;
			}
			case 21:{
				serviceTime = this.accurateSuperior(q_);
				break;
			}
			case 22:{
			...
			case 23:{
				serviceTime = this.piecewiseSuperior(q_);
				break;
			}
			case 33:{//combinaison
		...
		//choose the type : 1 -> one miss; 2 -> two miss; 3 -> combination
		switch (type)
		{
			case 3:{ 
				double[] m_t_1 = this.computeAverageMandTime(1, q);
				double[] m_t_2 = this.computeAverageMandTime(2, q);
				double proba = Math.pow(0.5, m_t_1[0]) * 2;
				timeService = m_t_1[1] * proba + m_t_2[1] * (1 - proba);  
				break;
			}
			default:{
				timeService = this.computeAverageMandTime(type, q)[1];
			}
		}
		
		return 1/timeService;
		*/
	}
