 ...
 *
 */
public class Solver1LimitedPolling {
	...
	/**
	 * Print the mu getten from the simulation
	 */
	public void printMus()
	{
		...
		for(int i = 0 ; i < N ; i++)
		{
			System.out.print("ATST("+(i+1)+") = "+1/this.queues.get(i).getMu()+" ; "); //average total service time
		}
		System.out.println("");
	}
	...
	/**
	 * 
	 * @return all the queues' blocking
	 */
	public Vector<Double> getB()
	{
		Vector<Double> bs = new Vector<Double>();
		for ( QueueInfo q : this.queues)
		{
			bs.add(q.getState(q.getK(), 0).getProba() + q.getState(q.getK(), 1).getProba());
		}
	...
	public double computeEpsilonArbitrary(int i) {
		...
		if (result >= 1) result = ( result + 1 )/2;
		
		q.setEpsilon(result);
		
		System.out.println("[computeEpsilonArbitrary] E"+i+" estimate = "+result);
		System.out.println("[compOLDEpsilonArbitrary] E"+i+" estimate = "+oldvalue);
		return result;
	}
	...
	public void run(double eps, boolean verbose_mode, int limit) {
		...
		//while (!this.checkCriteria(eps) && (nb_iter<5)) {
			nb_iter++;
			this.iterate();
			if (verbose_mode) {
	...
	private class QueueInfo {
		...
		public QueueInfo(int K_, double lambda_, double mu_) {
			...
			this.epsilon = 1 ; //Initialized with 1
		...
		public double getMaxVar() {
			...
			for (int k = 0 ; k<this.K+1 ; k++) {
				...
				test = Math.abs(this.oldStates[k][1].getProba() - this.states[k][1].getProba())/this.oldStates[k][1].getProba();
				if (test>result) {
					result = test;
		...
		/**
		 * Set the value of the epsilon estimate
		 * @para; value new value
		 */
		...
		{
			this.getState(0, 0).setProba(value);
		}
