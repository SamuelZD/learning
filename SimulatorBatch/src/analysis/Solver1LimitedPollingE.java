 ...
 *
 */
public class Solver1LimitedPollingE {
	...
	{
		//just the 3th queueu
		QueueInfo q = this.queues.get(0);
		double sum = 1 - q.getState(0, 0).getProba() - q.getState(0, 1).getProba() ;
		
		String fileAddr = System.clearProperty("user.dir") + "/traces/" + tracefile + "Proba";
		
		try
		{
			boolean earse = false; //true
			FileWriter filewriter = new FileWriter(fileAddr, !earse);
			BufferedWriter output = new BufferedWriter(filewriter);
			for(int k = 0 ; k <= q.getK() ; k ++)
			{
				String outputstring = "";
				if (k == 0) outputstring = "0 0" + "\n";
				else outputstring = k + " " + (q.getState(k, 0).getProba() + q.getState(k, 1).getProba())/sum + "\n";
				output.write(outputstring);
				output.flush();
			}
