 ...
 *
 */
public class Solver1LimitedPollingSwitch2 {
	...
	public Solver1LimitedPollingSwitch2(double switchRate_) {
		this.N = 0;
	...
	public void printSum()
	{
		for (int i = 0 ; i < N ; i++)
		{
			double sum = this.queues.get(i).getState(0,0).getProba() + this.computeProcessedProbas();
	...
	public double computeAlphaArbitrary(int i) {
		...
		q = this.queues.get(i);
		//double result = 1.0/( sum/( 1-q.getG() ) + this.N*1/this.switchRate);
		double result = 1.0/( sum + this.N/this.switchRate);
	...
	private class QueueInfo {
		 ...
		 */
		/*
		public void limit(int k, int m)
		{
			if(this.states[k][m].getProba() >= 1)
			{
				this.states[k][m].setProba( (this.oldStates[k][m].getProba()+1)/2 );
			}
			
			if(this.states[k][m].getProba() <= 0)
			{
				this.states[k][m].setProba( this.oldStates[k][m].getProba()/2 );
			}
		}
		*/
		...
		public void solve() {			
			...
			//double p00 = this.getState(0, 0).getProba();
			
			//double p01 = 1.0/ this.getGamma()*p00*this.getSystemInputLoad();
			double p01 = this.getState(0, 1).getProba();
			
			this.getState(0,1).setProba(p01);
			System.out.println("Proba PI_O,E = "+p01);
			
			double p10 = this.getState(0, 1).getProba()*this.lambda/this.mu;
			this.getState(1,0).setProba(p10);
			System.out.println("Proba PI_1,P = "+p10);
			
			double p11 =( this.getState(0, 1).getProba() + this.getState(1,0).getProba() )*this.lambda / this.getAlpha();
			...
			for (int k = 2; k<this.K ;k++) {
				...
				double pk1 = (this.getState(k-1,1).getProba() + this.getState(k,0).getProba())*this.getLambda()/this.getAlpha();
