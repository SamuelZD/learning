 ...
 * 
 * The first step : the inflection lambda == max mu
 * 
 * infini, proposed by Zidong.  If i have the time, I will finish it
 */
public class Solver1LimitedPollingELI {
	...
	public Solver1LimitedPollingELI() {
		this.N = 0;
		this.queues = new Vector<QueueInfo>();
	}
	
	public Solver1LimitedPollingELI(int m, double tm, double th, double tr) {
		this.N = 0;
		this.queues = new Vector<QueueInfo>();
		this.M = m;
		this.timeH = th;
	...
	public void printMuTime()
	{
		for (int i = 0 ; i < N ; i++)
		{
			System.out.print("Mu_Time(" + (i + 1) + ")" + 1/this.queues.get(i).getMu() + " ; ");
		}
		System.out.println("");
	}
	...
	public void init() {
		for (int i = 0; i<N ; i++) {
			...
			//init the mu
			double mu = this.computeProcessingRate();
			this.queues.get(i).setMu(mu);
		}
	}
	...
	/**
	 * Calculating the mu by using the linear line; Max mu == inflection lambda
	 *
	 */
	public double computeProcessingRate()
	{
		
		double iniMu = 1/(this.timeM + this.timeR);
		double servicetime =  ( (Configuration.N -1)* this.timeM + (M + 1 - Configuration.N)*this.timeH ) / Configuration.N;
		double maxMu = 1/( servicetime + this.timeR );
		
		double mu = 0 ;
		mu = (maxMu - iniMu)/maxMu * this.computeSystemInputLoad() + iniMu;
