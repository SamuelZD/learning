...
public class LotServerWithCacheExhaust extends HeterogeneousServer {
	
	private boolean one = true;
	
	
	public LotServerWithCacheExhaust(){
		super();
	...
	public void switchQueue(QDES simu, Queue currentQueue) {
		 ...
		 * ? < ? 
		 */
		if(currentQueue.getFirstWaitingUnholdCustomer() != null && currentQueue.getCountExhauste() < Configuration.EXTRALOT)
		{
			//add count
			currentQueue.setCountExhauste(currentQueue.getCountExhauste()+1);
			//end of vacation
			currentQueue.endVacation(simu);
			//serving again the current queue
			this.serveQueue(simu, currentQueue);
			
		}
		else // serving the next queue
		{
			// Start vacation  TODO : should be modified
			if (!currentQueue.inVacation()) 
			{ // At least one customer waiting to be served
							
				currentQueue.startVacation(simu);
			}
			//reset the values of the current queue
			currentQueue.setCountExhauste(0);
