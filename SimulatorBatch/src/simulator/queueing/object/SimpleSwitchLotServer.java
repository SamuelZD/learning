...
/**
 * Backup ? and fault in the reintialization
 */
...
public class SimpleSwitchLotServer extends HeterogeneousServer {
	
	private boolean one = true;
	private Distribution switchDistribution;
	
	
	public SimpleSwitchLotServer(){
		super();
	...
	public void processSwitch(QDES simu, Queue currentQueue)
	{
		...
		if (nextQueue.getFirstWaitingUnholdCustomer() != null) { // Serve the customer
			//System.out.println("the client is served");
			// At least one customer waiting to be served
			nextQueue.endVacation(simu);
			this.serveQueue(simu, nextQueue);
